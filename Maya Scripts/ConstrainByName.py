...
def main():
    ...
    sourceDescendents = cmd.listRelatives(sourceRoot, allDescendents=True)
    destDescendents = cmd.listRelatives(destRoot, allDescendents=True)
    ...
    for srcDesc in sourceDescendents:
        ...
        try:
            srcDesc_trim = TrimPrefixes(srcDesc)
        except:
        ...
        for destDesc in destDescendents:
            # Trim the name
            try:
                destDesc_trim = TrimPrefixes(destDesc)
            except:
                # If one of the names is irregularly short, it's not going to be a match, so skip it
                continue
            ...
            if srcDesc_trim == destDesc_trim:
                ...
                print("Constraining " + destDesc + " to " + srcDesc)
                cmd.parentConstraint(destDesc, srcDesc, maintainOffset=True)
                break
    return
